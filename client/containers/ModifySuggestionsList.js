...
function ModifySuggestionsList({ navigation: { goBack } }) {
  ...
  const fullSuggestionsList = useSelector(
    (state) => state.settings.suggestionSettings.fullSuggestionsList,
  );
  ...
  return (
    <View style={styles.container}>
      <View style={{ alignItems: 'center' }}>
        <MediumAppText style={{ marginTop: 25 }}>
          CUSTOMISE YOUR DAILY SUGGESTIONS
        </MediumAppText>
      </View>
      <ScrollView contentContainerStyle={{ alignItems: 'center' }}>
        {fullSuggestionsList.map((suggestion) => (
