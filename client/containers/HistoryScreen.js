...
function HistoryScreen() {
  ...
  useEffect(() => {
    ...
    if (requiredInfo.length)
      dispatch({
        ...
        payload: requiredInfo[0].moods.map(
  ...
  const suggestionsArr = useSelector(
    (state) => state.calendarHistoricalDay.suggestions,
  );
  ...
  const fullSuggestionsList = useSelector(
    (state) => state.settings.suggestionSettings.suggestionsList,
  );
  const unusedSuggestions = fullSuggestionsList.filter(
    (el) => !suggestionsArr.includes(el),
  );
  ...
  function clickHandler() {
    ...
    (localObj.date = DateTime.fromMillis(selectedDate)
      .toUTC()
      .startOf('day').ts),
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.infoWrapper}>
        <View style={styles.infoSubWrapper}>
          ...
          <View style={styles.iconWrapper}>
            <TouchableOpacity
              style={styles.icon}
              onPress={() => onMeetingsMinus()}
            >
              <Image source={require('../assets/remove.png')} />
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.icon}
              onPress={() => onMeetingsAdd()}
            >
        ...
        <View style={styles.infoSubWrapper}>
          <View style={styles.textWrapper}>
            ...
            <View style={{ flexDirection: 'row' }}>
              ...
              <BoldAppText style={{ marginTop: -3, fontSize: 14 }}>
                {' '}
                / 10
              </BoldAppText>
            </View>
          ...
          <View style={styles.iconWrapper}>
            <TouchableOpacity
              style={styles.icon}
              onPress={() => onFeelingMinus()}
            >
              <Image source={require('../assets/remove.png')} />
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.icon}
              onPress={() => onFeelingAdd()}
            >
              <Image source={require('../assets/add.png')} />
            </TouchableOpacity>
          </View>
        ...
        <View style={{ flex: 1, minHeight: 200 }}>
          ...
          <View>
            <MediumAppText style={{ marginBottom: 15, maxWidth: 200 }}>
              Suggestions completed:
            </MediumAppText>
            {suggestionsArr.map((suggestion) => (
              <HistoricalSuggestionItem
                key={suggestion}
          ...
          <View style={styles.unusedSuggestionsWrapper}>
            {unusedSuggestions.map((suggestion) => (
              <HistoricalUnusedSuggestionItem
                key={suggestion}
                name={suggestion}
      ...
      <View style={styles.wrapper}>
        <TouchableOpacity style={styles.button} onPress={() => clickHandler()}>
          <BoldAppText
            style={{ fontSize: 14, color: colors.cosmicLatte, marginBottom: 0 }}
          >
            UPDATE YOUR DAY
          </BoldAppText>
        </TouchableOpacity>
